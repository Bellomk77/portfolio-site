...
function StatusBadge({ status }: { status: FileStatus }) {
  const config = {
    queued:    { label: 'Queued',    className: 'bg-gray-800 text-gray-400 border-gray-700' },
    uploading: { label: 'Uploading', className: 'bg-blue-950/60 text-blue-400 border-blue-800' },
    analyzing: { label: 'AI Analysing', className: 'bg-purple-950/60 text-purple-400 border-purple-800' },
    done:      { label: 'Analysed', className: 'bg-ng-950/60 text-ng-400 border-ng-800' },
    error:     { label: 'Error',    className: 'bg-red-950/60 text-red-400 border-red-800' },
  }[status]
...
function ImplicationCard({ impl }: { impl: TaxImplication }) {
  const cfg = {
    action:  { border: 'border-ng-800/60', bg: 'bg-ng-950/40', icon: <ArrowRight className="w-3.5 h-3.5 text-ng-400" />, text: 'text-ng-300' },
    warning: { border: 'border-orange-800/60', bg: 'bg-orange-950/40', icon: <AlertCircle className="w-3.5 h-3.5 text-orange-400" />, text: 'text-orange-300' },
    info:    { border: 'border-blue-800/60', bg: 'bg-blue-950/40', icon: <CheckCircle className="w-3.5 h-3.5 text-blue-400" />, text: 'text-blue-300' },
  }[impl.severity]
    ...
    <div className={`rounded-xl border p-3.5 ${cfg.border} ${cfg.bg}`}>
      <div className="flex items-start gap-2.5">
        ...
        <div>
          ...
          <div className="text-gray-300 text-xs mb-1.5">{impl.description}</div>
          {impl.amount && <div className="text-white text-sm font-bold mb-1.5">{formatCurrency(impl.amount)}</div>}
          <div className="text-gray-400 text-xs bg-black/20 rounded-lg px-3 py-2 leading-relaxed">
            → {impl.action}
          </div>
        </div>
      </div>
    ...
    <div className="flex min-h-screen bg-gray-950">
      ...
      <main className="flex-1 ml-64 min-h-screen">
        <header className="sticky top-0 z-30 bg-gray-950/90 backdrop-blur border-b border-gray-800 px-8 py-4">
          <h1 className="text-xl font-bold text-white">AI Document Upload</h1>
        ...
        <div className="p-8 grid grid-cols-3 gap-6">
          ...
          <div className="col-span-1 space-y-5">
            ...
            >
              ...
              <div className="flex flex-wrap justify-center gap-1.5 text-xs text-gray-600">
                {['PDF', 'Excel', 'PNG/JPG', 'CSV'].map(t => (
                  <span key={t} className="bg-gray-800 px-2 py-0.5 rounded">{t}</span>
                ))}
              </div>
            ...
            {/* Summary stats */}
            {doneFiles.length > 0 && (
              <div className="bg-ng-950/30 border border-ng-900/50 rounded-xl p-4 space-y-2">
                ...
                <div className="flex justify-between text-xs">
                  <span className="text-gray-400">Documents analysed</span>
                  <span className="text-white font-semibold">{doneFiles.length}</span>
                </div>
                {totalVAT > 0 && (
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-400">Total VAT identified</span>
                    <span className="text-ng-400 font-semibold">{formatCurrency(totalVAT)}</span>
                  </div>
                )}
                <div className="flex justify-between text-xs">
                  <span className="text-gray-400">Action items found</span>
              ...
              <div className="space-y-2">
                ...
                {files.map(uf => (
                  <div
                    ...
                    className={cn(
                      'bg-gray-900 border rounded-xl p-3.5 transition-all',
                      uf.status === 'done' ? 'cursor-pointer hover:border-gray-600' : 'cursor-default',
                      selectedId === uf.id ? 'border-ng-700 bg-ng-950/20' : 'border-gray-800'
                    )}
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-start gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm text-white truncate font-medium">{uf.file.name}</div>
          ...
          <div className="col-span-2">
            {selectedFile?.result ? (
              <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                ...
                <div className="px-6 py-5 border-b border-gray-800 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-purple-600/20 border border-purple-800/50 rounded-xl flex items-center justify-center">
                      <Brain className="w-5 h-5 text-purple-400" />
                    </div>
                    <div>
                      <div className="text-white font-semibold">{selectedFile.result.documentType}</div>
                      <div className="text-gray-500 text-xs">{selectedFile.file.name}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs bg-ng-950/60 text-ng-400 border border-ng-800/50 px-2.5 py-1 rounded-full">
                      {selectedFile.result.confidence}% confidence
                    </span>
                    <button className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-white px-3 py-1.5 rounded-lg hover:bg-gray-800 transition-all border border-gray-700">
                ...
                <div className="p-6 grid grid-cols-2 gap-6">
                  {/* Extracted data */}
                  <div>
                    <h3 className="text-white font-semibold mb-4 text-sm">Extracted Document Data</h3>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center py-20">
                ...
                <div className="text-gray-400 font-semibold text-lg mb-2">AI Document Analyser Ready</div>
                ...
                <div className="mt-6 grid grid-cols-2 gap-3 max-w-xs">
                  {[
                    ...
                    <div key={f} className="flex items-center gap-2 text-xs text-gray-500 bg-gray-900/50 border border-gray-800 rounded-xl px-3 py-2">
