...
const FEATURES = [
  {
    ...
    color: 'bg-ng-950/80 text-ng-400 border-ng-900',
  ...
  },
  {
    ...
    color: 'bg-blue-950/80 text-blue-400 border-blue-900',
  ...
  },
  {
    ...
    color: 'bg-orange-950/80 text-orange-400 border-orange-900',
  ...
  },
  {
    ...
    color: 'bg-red-950/80 text-red-400 border-red-900',
  ...
  },
  {
    ...
    color: 'bg-yellow-950/80 text-yellow-400 border-yellow-900',
...
const typeColor = (type: string) => {
  switch (type) {
    case 'critical': return 'bg-red-950/50 text-red-400 border border-red-900'
    case 'new':      return 'bg-ng-950/50 text-ng-400 border border-ng-900'
    case 'alert':    return 'bg-orange-950/50 text-orange-400 border border-orange-900'
    default:         return 'bg-blue-950/50 text-blue-400 border border-blue-900'
  }
}
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      {/* ── Navbar ───────────────────────────────────────────── */}
      <header className="fixed top-0 inset-x-0 z-50 bg-gray-950/85 backdrop-blur-xl border-b border-gray-800/60">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <span className="text-white font-bold text-lg">TaxPadi</span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-3">
            <Link href="/dashboard" className="text-gray-400 hover:text-white text-sm px-4 py-2">Login</Link>
            <Link href="/dashboard" className="bg-ng-600 hover:bg-ng-500 text-white text-sm px-5 py-2 rounded-lg font-medium shadow-lg shadow-ng-600/30 transition-all hover:shadow-ng-500/40">
              Start Free Trial
            </Link>
          </div>

          {/* Mobile menu */}
          <button className="md:hidden text-gray-400" onClick={() => setMobileOpen(v => !v)}>
            {mobileOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
          </button>
        </div>

        {mobileOpen && (
          <div className="md:hidden border-t border-gray-800 bg-gray-950 px-5 py-4 space-y-3">
            {NAV_LINKS.map(l => (
              <a key={l.href} href={l.href} onClick={() => setMobileOpen(false)}
                className="block text-gray-300 hover:text-white py-1.5">{l.label}</a>
            ))}
            <Link href="/dashboard" className="block mt-3 bg-ng-600 text-white text-center py-3 rounded-xl font-medium">
              Start Free Trial
            </Link>
          </div>
        )}
      ...
      <section className="relative pt-32 pb-24 px-5 overflow-hidden">
        {/* Background glow */}
        <div className="absolute inset-0 bg-gradient-to-b from-ng-950/30 via-transparent to-transparent pointer-events-none" />
        <div className="absolute -top-40 left-1/3 w-[600px] h-[600px] bg-ng-600/6 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-blue-900/8 rounded-full blur-3xl pointer-events-none" />

        <div className="relative max-w-5xl mx-auto text-center">
          {/* Badge */}
          <div className="inline-flex items-center gap-2 bg-ng-950/70 border border-ng-800/60 text-ng-400 px-4 py-1.5 rounded-full text-sm mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
            <Link
              href="/dashboard"
              className="inline-flex items-center justify-center gap-2 bg-ng-600 hover:bg-ng-500 text-white px-8 py-4 rounded-xl text-base font-semibold transition-all hover:shadow-xl hover:shadow-ng-600/30"
            >
              Start for Free — No Card Needed <ArrowRight className="w-5 h-5" />
            </Link>
            <Link
              href="/calculator"
              className="inline-flex items-center justify-center gap-2 border border-gray-700 hover:border-gray-500 text-gray-300 hover:text-white px-8 py-4 rounded-xl text-base transition-all"
            >
              Try Tax Calculator
            </Link>
          </div>
      ...
      {/* ── Scrolling ticker ─────────────────────────────────── */}
      <div className="py-4 bg-gray-900/40 border-y border-gray-800 overflow-hidden">
      ...
      <section id="features" className="py-24 px-5">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-white mb-4">Everything Your Nigerian Business Needs</h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURES.map((f, i) => (
              <div key={i} className={`rounded-2xl p-6 bg-gray-900 border border-gray-800 hover:border-gray-700 transition-all group`}>
      ...
      {/* ── How it Works ─────────────────────────────────────── */}
      <section className="py-24 px-5 bg-gray-900/30 border-y border-gray-800">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-white mb-4">Tax Filing in 3 Simple Steps</h2>
            <p className="text-gray-400 text-lg">No accounting degree. No manual entries. TaxPadi does the heavy lifting.</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {STEPS.map((s, i) => (
              <div key={i} className="relative">
                ...
                <div className="w-11 h-11 bg-ng-600/20 rounded-xl flex items-center justify-center mb-4 border border-ng-800">
      ...
      <section id="updates" className="py-24 px-5">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <div>
              <div className="inline-flex items-center gap-2 bg-orange-950/50 border border-orange-800/60 text-orange-400 px-3 py-1.5 rounded-full text-sm mb-6">
                <AlertCircle className="w-3.5 h-3.5" />
                Live Tax Intelligence
              </div>
              <h2 className="text-4xl font-bold text-white mb-4">Always Current with Nigerian Tax Law</h2>
              ...
              <div className="space-y-3">
                {TAX_UPDATES.map((u, i) => (
                  <div key={i} className="flex items-start gap-3 bg-gray-900 rounded-xl p-4 border border-gray-800">
            ...
            <div className="bg-gray-900 rounded-2xl p-6 border border-gray-800">
              <div className="flex items-center gap-2 mb-5">
                ...
                <span className="text-gray-400 text-sm">AI Knowledge Base — Last Updated Today</span>
              </div>
              <div className="space-y-2">
                {LAWS.map(law => (
                  <div key={law} className="flex items-center justify-between p-3 bg-gray-800/60 rounded-xl">
                    <div className="flex items-center gap-2.5">
                      <FileText className="w-3.5 h-3.5 text-gray-500 flex-shrink-0" />
                      <span className="text-gray-300 text-sm">{law}</span>
                    </div>
      ...
      <section className="py-24 px-5 bg-gray-900/30 border-y border-gray-800">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-4xl font-bold text-white mb-4">Trusted by Nigerian Businesses</h2>
            <div className="flex items-center justify-center gap-1 text-yellow-400">
              {[...Array(5)].map((_, i) => <Star key={i} className="w-4 h-4 fill-current" />)}
              <span className="text-gray-400 text-sm ml-2">4.9/5 from 2,300+ reviews</span>
            </div>
          </div>
            ...
            {TESTIMONIALS.map((t, i) => (
              <div key={i} className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-ng-600/30 border border-ng-800 rounded-full flex items-center justify-center text-ng-400 text-sm font-bold">
                    {t.avatar}
                  </div>
                  <div>
      ...
      <section id="pricing" className="py-24 px-5">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-gray-400 text-lg">Less than one hour with a tax consultant. Cancel anytime.</p>
          </div>
            ...
            {PRICING.map((p, i) => (
              <div key={i} className={`rounded-2xl p-6 relative ${p.featured
                ...
                <div className="flex items-baseline gap-1.5 mb-1">
                  <span className="text-4xl font-extrabold text-white">₦{p.priceNGN}</span>
                ...
                <p className={`text-sm mb-5 ${p.featured ? 'text-ng-100' : 'text-gray-400'}`}>{p.desc}</p>
                <ul className="space-y-2.5 mb-6">
                  {p.features.map(f => (
                    <li key={f} className="flex items-start gap-2 text-sm">
                      <CheckCircle className={`w-4 h-4 flex-shrink-0 mt-0.5 ${p.featured ? 'text-ng-200' : 'text-ng-500'}`} />
                      <span className={p.featured ? 'text-ng-50' : 'text-gray-300'}>{f}</span>
                    </li>
                  ))}
                </ul>
                <Link
      ...
      <section className="py-24 px-5">
        <div className="max-w-4xl mx-auto">
          <div className="bg-gradient-to-br from-ng-950 via-gray-900 to-gray-950 rounded-3xl p-12 sm:p-16 border border-ng-900 text-center relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <Link
                href="/dashboard"
                className="inline-flex items-center gap-3 bg-ng-600 hover:bg-ng-500 text-white px-10 py-4 rounded-xl text-lg font-bold transition-all hover:shadow-2xl hover:shadow-ng-600/40"
              >
                Start Your 30-Day Free Trial <ArrowRight className="w-5 h-5" />
              </Link>
              <div className="flex items-center justify-center gap-5 mt-8 text-gray-500 text-sm">
                <div className="flex items-center gap-1.5"><CheckCircle className="w-4 h-4 text-ng-600" />No credit card</div>
                <div className="flex items-center gap-1.5"><Lock className="w-4 h-4 text-ng-600" />Bank-grade security</div>
                <div className="flex items-center gap-1.5"><Globe className="w-4 h-4 text-ng-600" />All Nigerian states</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ── Footer ────────────────────────────────────────────── */}
      <footer className="py-14 px-5 border-t border-gray-800">
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-4 gap-10 mb-10">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-white font-bold">TaxPadi</span>
              </div>
            ...
            {[
              ...
              <div key={col.title}>
                ...
                <ul className="space-y-2">
                  {col.links.map(l => (
                    <li key={l}><Link href="/dashboard" className="text-gray-500 text-sm hover:text-gray-300 transition-colors">{l}</Link></li>
                  ))}
                </ul>
              </div>
            ))}
          ...
          <div className="border-t border-gray-800 pt-8 flex flex-col md:flex-row items-center justify-between gap-3 text-xs text-gray-600">
