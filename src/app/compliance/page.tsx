...
function DeadlineRow({ deadline }: { deadline: typeof ALL_DEADLINES_2025[number] }) {
  ...
  return (
    <div className="border border-gray-800 rounded-xl overflow-hidden transition-all">
      <div
        className="flex items-center gap-4 px-5 py-4 hover:bg-gray-800/20 cursor-pointer"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="text-white text-sm font-medium truncate">{deadline.description}</div>
        ...
        <div className="text-right flex-shrink-0">
          <div className="text-white text-sm font-bold">{deadline.amount ? formatCurrency(deadline.amount) : '—'}</div>
      ...
      {expanded && (
        <div className="px-5 pb-4 border-t border-gray-800/60 pt-4 bg-gray-900/30">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-gray-900 rounded-xl p-3.5 border border-gray-800">
              ...
              <div className="text-white font-bold">{deadline.amount ? formatCurrency(deadline.amount) : '—'}</div>
            </div>
            <div className="bg-gray-900 rounded-xl p-3.5 border border-gray-800">
          ...
          {deadline.status !== 'filed' && deadline.amount && days >= 0 && (
            <div className="mt-3 bg-orange-950/30 border border-orange-900/50 rounded-xl p-3 text-xs text-orange-300">
          ...
          {deadline.status === 'overdue' && (
            <div className="mt-3 bg-red-950/30 border border-red-900/50 rounded-xl p-3 text-xs text-red-300">
      ...
      <main className="flex-1 ml-64 min-h-screen">
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              { label: 'Compliance Rate', value: `${complianceRate}%`, sub: `${filed}/${ALL_DEADLINES_2025.length} filings`, color: 'text-ng-400', bg: 'bg-ng-950/40 border-ng-900' },
              { label: 'Filed On Time',    value: filed.toString(),   sub: 'total filings',     color: 'text-ng-400',     bg: 'bg-ng-950/40 border-ng-900' },
              { label: 'Pending',          value: pending.toString(), sub: 'need attention',     color: 'text-yellow-400', bg: 'bg-yellow-950/40 border-yellow-900' },
              { label: 'Overdue',          value: overdue.toString(), sub: 'act immediately',    color: 'text-red-400',    bg: 'bg-red-950/40 border-red-900' },
            ].map(card => (
          ...
          <div className="flex items-center gap-3">
              ...
              {(['all', 'pending', 'filed'] as const).map(s => (
                <button key={s} onClick={() => setFilterStatus(s)}
                  className={cn('px-4 py-2 rounded-lg text-sm font-medium capitalize transition-all',
                    filterStatus === s ? 'bg-ng-600 text-white' : 'text-gray-400 hover:text-white'
                  )}>
                  {s}
                </button>
              ))}
            ...
            <select
              ...
              className="bg-gray-900 border border-gray-800 text-gray-300 text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-ng-600"
            >
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
            <h3 className="text-white font-semibold mb-4">FIRS Penalty Reference Guide</h3>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-800/50">
                  {[
                    ...
                    <tr key={row.tax}>
                      <td className="py-3 pr-6 text-white font-medium">{row.tax}</td>
